...
class GradNorm(nn.Module):
    ...
    def forward(self, flow):
        ...
        if self.reduction == "none":
            # Sim channels. Keep spatial dimensions
...
class DiceOverlap(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        for label in self.classes:
            ...
            if union == 0:
                # overlap of empty sets is 1
                dice_overlap = torch.as_tensor(1., device=union.device)
            else:
...
class SoftDiceOverlap(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        # sum over B, D, H, W
        sum_dims = [0, 2, 3] if torchreg.settings.get_ndims() == 2 else [
            0, 2, 3, 4]
